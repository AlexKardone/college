  ...
  _createClass(Search, [{
    ...
    value: function getResults() {
      ...
      _jquery.default.getJSON(universityData.root_url + '/wp-json/university/v1/search?term=' + this.searchField.val(), function (results) {
        _this.resultsDiv.html("\n\t\t\t\t<div class=\"row\">\n\t\t\t\t\t<div class=\"one-third\">\n\t\t\t\t\t\t<h2 class=\"search-overlay__section-title\">General Information</h2>\n\t\t\t\t\t\t".concat(results.generalInfo.length ? '<ul class="link-list min-list">' : '<p>No general information match that search.</p>', "\n\t\t\t\t\t\t\t").concat(results.generalInfo.map(function (item) {
        ...
        }).join(''), "\n\t\t\t\t\t\t").concat(results.generalInfo.length ? '</ul>' : '', "\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class=\"one-third\">\n\t\t\t\t\t\t<h2 class=\"search-overlay__section-title\">Programs</h2>\n\t\t\t\t\t\t").concat(results.programs.length ? '<ul class="link-list min-list">' : "<p>No programs information match that search. <a href=\"".concat(universityData.root_url, "/programs\">Veiw all programs</a></p>"), "\n\t\t\t\t\t\t\t").concat(results.programs.map(function (item) {
          ...
          return "\n\t\t\t\t\t\t\t\t<li class=\"professor-card__list-item\">\n\t\t            \t<a class=\"professor-card\" href=\"".concat(item.permalink, "\">\n\t\t            \t\t<img class=\"professor-card__image\" src=\"").concat(item.image, "\" alt=\"<?php the_title(); ?>\">\n\t\t            \t\t<span class=\"professor-card__name\">").concat(item.title, "</span>\n\t\t            \t</a>\n\t\t            </li>\n\t\t\t\t\t\t\t");
        }).join(''), "\n\t\t\t\t\t\t").concat(results.professors.length ? '</ul>' : '', "\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class=\"one-third\">\n\t\t\t\t\t\t<h2 class=\"search-overlay__section-title\">Campuses</h2>\n\t\t\t\t\t\t").concat(results.campuses.length ? '<ul class="link-list min-list">' : "<p>No campuses match that search. <a href=\"".concat(universityData.root_url, "/campuses\">Veiw all campuses</a></p>"), "\n\t\t\t\t\t\t\t").concat(results.campuses.map(function (item) {
          return "<li><a href=\"".concat(item.permalink, "\">").concat(item.title, "</a></li>");
        }).join(''), "\n\t\t\t\t\t\t").concat(results.campuses.length ? '</ul>' : '', "\n\n\t\t\t\t\t\t<h2 class=\"search-overlay__section-title\">Events</h2>\n\t\t\t\t\t\t").concat(results.events.length ? '' : "<p>No events match that search. <a href=\"".concat(universityData.root_url, "/events\">Veiw all events</a></p>"), "\n\t\t\t\t\t\t\t").concat(results.events.map(function (item) {
